...
async function getCurrentJson(inputLocation) {
  let fetchResult = await fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${inputLocation}/today?unitGroup=metric&include=current&key=CPDRKTZAL4B8C8GGBDKP2DCMA&contentType=json`,
  );
...
async function getHourlyJson(location, date) {
  let fetchResult = await fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${location}/${date}/${date}?unitGroup=metric&include=hours&key=CPDRKTZAL4B8C8GGBDKP2DCMA&contentType=json`,
  );
  let json = await fetchResult.json();
  return json;
}
...
async function getDailyJson(location, fromDate, toDate) {
  let response = await fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${location}/${fromDate}/${toDate}?unitGroup=metric&include=days&key=CPDRKTZAL4B8C8GGBDKP2DCMA&contentType=json`,
  );
